"""Exercise FSDP critic training with full, dedicated policy and critic GPUs.

The critic is explicitly requested through stock Tinker ``user_metadata``. The
client checks the clipped critic loss from forward/backward and the positive
finite gradient norm returned by the optimizer step. NumPy arrays keep the
critic-only ``values`` and ``returns`` on Tinker's generic tensor path.
"""
...
async def main() -> None:
    ...
    await service.create_lora_training_client_async(base_model="Qwen/Qwen3-0.6B", rank=8)
